...
The basic building block of a context-free syntax, lexical syntax or variables section is the production. The left-hand side of a productive rule can be either just a sort or a sort followed by `.` and a constructor name. The right-hand side consists of zero or more symbols. Both sides are separated by `=`:
...
Productions can also be written in reductive form, which consists of a left-hand side of zero or more symbols, followed by an arrow symbol `->` and a right-hand side that consists of one symbol:
